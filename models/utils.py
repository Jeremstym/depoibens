    ...
    def __call__(self, path, current_valid_loss, epoch, model, optimizer, criterion):
        if current_valid_loss < self.best_valid_loss:
            ...
            torch.save(
                ...
                path + "/outputs/best_model_pearson.pth",
            )
...
def save_model(path, epochs, model, optimizer, criterion):
    ...
    torch.save(
        ...
        path + "/outputs/final_model_pearson.pth",
    )
...
def save_plots(path, train_score, valid_score, train_loss, valid_loss):
    ...
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig(path + "/outputs/r2_weighted_pearson.png")
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig(path + "/outputs/loss_pearson.png")
